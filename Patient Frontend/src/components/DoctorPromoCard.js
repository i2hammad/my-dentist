...
// Doctor marketing campaign card — same design as the patient PromoCard.
// Drop it into any doctor screen's scroll content: <DoctorPromoCard />
export default function DoctorPromoCard({ style }) {
          ...
          return (
            ...
            >
              ...
              <View style={{ padding: 14 }}>
                <Text
                  style={{ color: '#FFF', fontWeight: '800', fontSize: 17, textShadowColor: 'rgba(0,0,0,0.3)', textShadowRadius: 4 }}
                  numberOfLines={1}
                >
                ...
                {!!(c.bannerText || c.body) && (
                  <Text
                    style={{ color: 'rgba(255,255,255,0.92)', fontSize: 12.5, marginTop: 3, textShadowColor: 'rgba(0,0,0,0.25)', textShadowRadius: 3 }}
                    numberOfLines={1}
                  >
                    {c.bannerText || c.body}
                  </Text>
                ...
                <View style={{
                  ...
                  backgroundColor: 'rgba(255,255,255,0.95)', borderRadius: 20, paddingHorizontal: 12, paddingVertical: 5,
                }}>
      ...
      {campaigns.length > 1 && (
        <View style={{ flexDirection: 'row', justifyContent: 'center', gap: 5, marginTop: 8 }}>
          {campaigns.map((_, i) => (
            <View
              key={i}
              style={{ width: i === idx ? 16 : 6, height: 5, borderRadius: 3, backgroundColor: i === idx ? '#0052FF' : '#CBD5E1' }}
            />
          ))}
        </View>
      )}
