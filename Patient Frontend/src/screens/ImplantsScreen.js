...
export default function ImplantsScreen({ navigation }) {
  ...
  const fetchDoctors = useCallback(async () => {
    try {
      ...
      if (res.data?.success) {
        // Filter doctors specialized in Implants
        const filtered = (res.data.data || []).filter(doc => 
          doc.specialization?.toLowerCase().includes('implant') ||
          doc.about?.toLowerCase().includes('implant')
        );
        setDoctors(filtered);
      }
    } catch (e) {
      console.log('Error fetching implants doctors:', e?.message);
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      <AnimatedHeader style={styles.blueHeader}>
        <View style={styles.headerRow}>
          ...
          <Text style={styles.headerTitle}>Dental Implants</Text>
          <View style={{ width: 24 }} />
        </View>
      </AnimatedHeader>
      ...
      >
        <View style={styles.introCard}>
          ...
          <Text style={styles.introText}>
            Find top implantology specialists for permanent tooth restoration, dental implants, and full mouth rehabilitation.
          </Text>
        </View>
        ...
        {loading ? (
          <View style={styles.loaderContainer}>
            ...
            <Text style={styles.loadingText}>Loading implant specialists…</Text>
          </View>
        ) : doctors.length === 0 ? (
          <View style={styles.emptyContainer}>
            <Ionicons name="sad-outline" size={48} color="#CBD5E1" />
            <Text style={styles.emptyText}>No implant specialists found right now.</Text>
