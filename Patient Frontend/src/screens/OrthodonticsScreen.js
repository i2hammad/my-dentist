...
export default function OrthodonticsScreen({ navigation }) {
  ...
  const fetchDoctors = useCallback(async () => {
    try {
      ...
      if (res.data?.success) {
        // Filter doctors specialized in Orthodontics
        const filtered = (res.data.data || []).filter(doc => 
          doc.specialization?.toLowerCase().includes('ortho') ||
          doc.about?.toLowerCase().includes('ortho') ||
          doc.about?.toLowerCase().includes('brace') ||
          doc.about?.toLowerCase().includes('align')
        );
        setDoctors(filtered);
      }
    } catch (e) {
      console.log('Error fetching orthodontics doctors:', e?.message);
    } finally {
      setLoading(false);
    }
  }, []);
      ...
      >
        ...
        <View style={styles.introCard}>
          ...
          <Text style={styles.introText}>
            Find top orthodontists for dental alignment, braces, clear aligners, and bite correction treatments.
          </Text>
        </View>
        ...
        {loading ? (
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>No orthodontists found right now.</Text>
